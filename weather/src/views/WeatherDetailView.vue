...
// 라우터 ID 파라미터를 실제 OpenWeatherMap 쿼리용 영문 명칭과 한글 명칭으로 매핑
const cityMapping = {
...
// 🎨 날씨 상태(맑음/흐림/비/눈/야간)에 따라 정보 카드 색을 자동으로 바꿔주는 테마 클래스
const themeClass = computed(() => {
  if (!cityData.value) return ''
  return weatherThemeClass(cityData.value.status, {
    weatherId: cityData.value.weatherId,
...
<template>
  <div class="detail-container">
    ...
    <div v-if="isLoading" style="text-align: center; padding: 20px 0; color: #7f8c8d">상세 정보를 동기화하는 중입니다...</div>

    <template v-else>
      <div v-if="cityData" class="info-card" :class="themeClass">
